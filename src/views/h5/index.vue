<template>
  <div class="page">
    ...
    <div id="tabbar" :class="{'on': outAble }" v-if="btnShow&&(AppID !== 'qiandada87654321')">
      <!-- <div class="tabbaritem"><p>完成提额认证，可大幅提升借款额度</p></div> -->
...
// 10 未认证 ; 11 认证中 ; 12 已失效；20 已认证; 30 失败; 21驳回
    ...
    indexApi.getIndex()
      .then((response) => {
        ...
        if (res.code === 200) {
          ...
          console.log(this.authList.baseAuth, 'authlist')
          // 基础认证排序
          ...
          // 把未认证其他项 和 未认证过的身份认证 的选出来 运营商为已失效也要加进去 *添加驳回状态
          let noStatusArr
          noStatusArr = this.authList.baseAuth.filter((item) => {
            if (item.verifyType === 'statusIdentityCertification') { // 循环过程中将 身份实名认证中 配置项传入query
              this.getIdentityCertificationItems(item.items)
            }
            return item.verifyStatus === STATUS_FAIL || item.verifyStatus === STATUS_UNABLE || item.verifyStatus === STATUS_REJECT || (item.verifyStatus === STATUS_SUCESS && (item.verifyStatusLive && item.verifyStatusLive !== STATUS_SUCESS))
          })
  ...
  methods: {
    ...
    */
    ...
    filterToText (item) {
      ...
      if (item.verifyStatus === STATUS_SUCESS) {
        return '已认证'
      } else if (item.verifyStatus === STATUS_FAIL || item.verifyStatus === STATUS_REJECT) { // *添加驳回
        return '去认证'
    ...
    filterToClass (item) { // 判断颜色
      ...
      // 各项失败及其身份认证第一步为成功,第二部为失败的情况 *添加驳回
      if (item.verifyStatus === STATUS_FAIL || item.verifyStatus === STATUS_REJECT || (item.verifyStatus === STATUS_SUCESS && (item.verifyStatusLive && item.verifyStatusLive !== STATUS_SUCESS))) {  // 按顺序查找未认证然后第一个为蓝色默认为灰色
        if (this.confuseIs) {
          return ''
        }
    ...
    getIdentityCertificationItems (item) { // 获取
      console.log('getitem')
      this.$route.query.picFrontpicFront = false
      this.$route.query.picBack = false
      this.$route.query.picHand = false
      this.$route.query.inputName = false
      this.$route.query.inputIdentify = false
      item.forEach(child => {
        switch (child.code) {
          case 'idetityFace':
            this.$route.query.picFront = true
            break
          case 'identityBack':
            this.$route.query.picBack = true
            break
          case 'autodyne':
            this.$route.query.picHand = true
            break
          case 'name':
            this.$route.query.inputName = true
            break
          case 'idCard':
            this.$route.query.inputIdentify = true
            break
        }
      })
    }
