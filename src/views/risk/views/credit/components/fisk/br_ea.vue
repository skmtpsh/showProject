    ...
    <dd>
      <el-row :gutter="60" style=" padding: 20px 0;">
        <el-col :span="8" v-for="item in bairongDetailData.brArr" :key="item.rule" style="margin:5px 0; text-align: left;">
          {{item.rule}}：
          <!-- <el-tag size="small" v-if="item.key.indexOf('EaIs') > -1" :type="item.value === '1' ? 'sucess' : 'danger'">{{item.value | FilterToBR(item.key)}}</el-tag> -->
          <b>{{item.value}}</b>
  ...
  computed: {
    ...
    bairongData () {
      ...
      let brTableData = Object.keys(brData).reduce((pre, cur) => {
        if ((/RuleNameRED(.*?)$/g).test(cur)) {
          let code = cur.match(/RED(.*?)$/g)
    ...
    bairongApp () {
      let brData = this.fiskData
      if (!brData.riskEaLoanChgnum) {
        return false
      }
      let days = [7, 30, 90, 180]
      return brData.riskEaLoanChgnum.split(',').reduce((pre, cur, i) => {
        pre[0]['day' + days[i]] = cur
        return pre
      }, [{}])
    }
  },
